...
conv_qwen_2_centerline_coord = Conversation(
    system=(
        "<|im_start|>system\n"
        "You are a BEV road-geometry expert. The image is a 512x512 RC-format BEV patch with colored road-boundary "
        "and lane-marking cues on a dark background.\n"
        "Infer only the drivable lane centerline polylines. Use the road boundaries and lane markings to estimate each "
        "lane center, even when cues are partially missing, broken, duplicated, or slightly overlapped by annotation noise.\n"
        "Output only compact JSON with image-local integer pixel coordinates. Do not describe the image or output "
        "road boundaries or lane markings. Start directly with [. Each object must be: "
        "{\"points\":[[x,y],...],\"category\":\"CenterLine\"}."
    ),
    roles=("<|im_start|>user\n", "<|im_start|>assistant\n"),
    version="qwen_v2",
    messages=(),
...
conv_qwen_2_Dinov2_huawei = Conversation(
    system=(
        "<|im_start|>system\n"
